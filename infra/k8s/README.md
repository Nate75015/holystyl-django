...
# Env applicatif (DATABASE_URL = Serverless SQL : user=access_key, pwd=secret_key)
DB_AK=$(terraform -chdir=infra/terraform/envs/preprod output -raw db_access_key)
DB_SK=$(terraform -chdir=infra/terraform/envs/preprod output -raw db_secret_key)
kubectl -n holystyl create secret generic holystyl-env \
  ...
  --from-literal=DATABASE_URL="postgres://$DB_AK:$DB_SK@xxxxx.pg.sdb.fr-par.scw.cloud:5432/holystyl?sslmode=require" \
...
- `SECURE_SSL_REDIRECT=false` le temps du 1er test HTTP ; repasser à `true`
  une fois le certificat TLS émis, puis `kubectl rollout restart deploy/web -n holystyl`.
